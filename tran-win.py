  ...
  for _type in TYPES:
    for _file in listOfFiles:
      if _file.endswith(_type):
        # print('ffprobe -v error -show_entries stream=codec_long_name \"' + _file + '\" | findstr codec_long_name')
        out = os.popen('cmd /c D:\\utilities\\ffmpeg\\bin\\ffprobe -v error -show_entries stream=codec_long_name \"' + _file + '\" | findstr codec_long_name').read()
