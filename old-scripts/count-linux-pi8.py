  ...
  for _type in TYPES:
    for _file in listOfFiles:
      if _file.endswith(_type):
        ...
        codec = popen('/usr/bin/ffprobe -v error -show_entries stream=codec_long_name \"' + _file + '\" | grep codec_long_name').read()
        
        if '265' in codec:
          if VERBOSE == "True":
            print('265 for ' + fileNameNoExt)
          global codec265Count
          codec265Count += 1
        elif '264' in codec:
          if VERBOSE == "True":
            print('264 for ' + fileNameNoExt)
        global totalVideos
        totalVideos += 1
  if VERBOSE == "True":
